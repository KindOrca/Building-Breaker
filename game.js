...
const building={
  ...
  resolvePlayerCollision(p,prevX){
    ...
    if(prevRight<=left+3){p.x=left-p.w/2-1;if(p.vx>0)p.vx=0;}
      ...
      // 건물이 내려와 플레이어를 덮는 경우 HP를 깎지 않고 가장 가까운 바깥쪽으로 밀어낸다.
  ...
  update(dt){
    ...
    if(this.bottom()>GROUND-38){dangerPulse+=dt*8;if(this.bottom()>GROUND+4){player.damage(true);this.descent-=72;shake=Math.max(shake,16);flash=Math.max(flash,.22);popup('GROUND IMPACT!',W/2,122,{size:32,color:'#ff6a5f',life:.6});}}
  },
  ...
  destroyFloor(i,meta){
    ...
    if(!this.floors.length){score+=2500*stage;gauge=clamp(gauge+15,0,100);this.buildingId++;stage++;freeze=Math.max(freeze,.14);shake=20;flash=.5;popup('TOTAL BREAK!',W/2,280,{size:58,color:'#fff37a',life:1.2});setTimeout(()=>{if(state==='play')this.spawn()},420)}
  },
  ultimateBreak(count){
...
function drawUI(c){
  ...
  if(tutorial>0&&state==='play'){const a=clamp(tutorial>1?1:tutorial,0,1);c.save();c.globalAlpha=a;c.fillStyle='rgba(0,0,0,.6)';c.beginPath();c.roundRect(W/2-330,112,660,72,18);c.fill();text('← → MOVE    ↑ JUMP    ↓ GUARD    Z ATTACK    X SPECIAL',W/2,142,24,'#ffffff');text('JUMP + Z  →  STRIKE UPWARD INTO THE GLOWING WEAK POINT!',W/2,174,18,'#ffd95a');c.restore()}
...
function drawHowTo(){
  ...
  const rows=[['← / →','MOVE','Position yourself under the weak point'],['↑','JUMP','Jump high enough to reach the building'],['↓','GUARD','Block falling debris / time it perfectly'],['Z','UP ATTACK','Strike straight upward / best after jumping'],['X','SPECIAL','Use when the gauge reaches 100%']];
