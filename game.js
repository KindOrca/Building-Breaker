...
const player={
  ...
  update(dt){
    ...
    if(this.dash>0){this.dash-=dt;this.vx=this.facing*560}
  ...
  attack(){
    ...
    this.attackCd=this.onGround?.15:.19;this.attackAnim=.13;
    if(!this.onGround)this.dash=.075;
    audio.tone(this.onGround?170:205,.045,'square',.04,60);
    const range=this.onGround?78:96;
    const ax=this.x+this.facing*(this.w/2+range*.48), ay=this.y-(this.onGround?4:3);
    const hit=building.tryHit(ax,ay,range,70,!this.onGround,this.attackStep);
    if(!hit){wave(ax,ay,28,.12);}
  },
  damage(){
    if(this.invuln>0)return;
    if(this.guard&&this.guardMeter>0){
      ...
      if(perfect){stats.perfectGuards++;gauge=clamp(gauge+6,0,100);popup('PERFECT GUARD!',this.x,this.y-82,{color:'#70e8ff',size:32});audio.perfect();freeze=Math.max(freeze,.055);this.perfectWindow=.0;return}
  ...
  draw(c){
    ...
    if(this.guard){c.save();c.globalAlpha=.32;c.fillStyle='#7fe6ff';c.beginPath();c.arc(this.facing*8,0,40,-1.4,1.4);c.fill();c.restore()}
    if(this.attackAnim>0){c.save();c.strokeStyle=this.hitGlow>0?'#fff7bf':'#ffd34d';c.lineWidth=12;c.lineCap='round';c.beginPath();c.arc(this.facing*28,-4,42,this.facing>0?-1.2:1.95,this.facing>0?.8:3.95);c.stroke();c.restore()}
...
const building={
  ...
  reset(){this.buildingId=1;this.style=0;this.spawn()},
  spawn(){
    ...
    this.pickWeak();popup(`BUILDING ${this.buildingId}`,W/2,92,{size:28,color:'#ffd45a',life:1.1});
  },
  ...
  update(dt){
    this.descent+=this.descentSpeed*dt;this.weakPulse+=dt*4.5;this.collapseKick=Math.max(0,this.collapseKick-dt*60);
    if(Math.random()<dt*(.22+stage*.025))spawnHazard();
    if(this.bottom()>GROUND-38){dangerPulse+=dt*8;if(this.bottom()>GROUND+4){player.damage();this.descent-=72;}}
  },
  ...
  damageFloor(i,dmg,meta){
    ...
    let comboAdd=1; combo++;comboTimer=2.8;gauge=clamp(gauge+1.4+(meta.perfect?5:0),0,100);score+=Math.floor(dmg*(1+comboMultiplier()));
    stats.maxCombo=Math.max(stats.maxCombo,combo);
    player.hitGlow=.08;freeze=Math.max(freeze,meta.perfect?.095:.038);shake=Math.max(shake,meta.perfect?13:3.5);zoomKick=Math.max(zoomKick,meta.perfect?.065:.018);flash=Math.max(flash,meta.perfect?.2:.055);
    wave(meta.x,meta.y,meta.perfect?118:64,meta.perfect?.3:.2);burst(meta.x,meta.y,meta.perfect?16:7,meta.perfect?1.25:.8,this.style===0?'#ba9f7b':'#aab3bb');audio.hit(meta.perfect?2:1);
  ...
  destroyFloor(i,meta){
    ...
    this.floors.splice(i,1);stats.destroyedFloors++;combo+=2;comboTimer=2.8;gauge=clamp(gauge+3.5,0,100);score+=Math.floor(480*(1+comboMultiplier()));
    freeze=Math.max(freeze,.085);shake=Math.max(shake,9);zoomKick=Math.max(zoomKick,.042);flash=Math.max(flash,.13);audio.break(1.4);burst(r.x+r.w/2,r.y+r.h/2,15,1.05,this.style===0?'#c7a57a':'#adb4bb');
    ...
    if(!this.floors.length){score+=2500*stage;gauge=clamp(gauge+10,0,100);this.buildingId++;stage++;freeze=Math.max(freeze,.14);shake=20;flash=.5;popup('TOTAL BREAK!',W/2,280,{size:58,color:'#fff37a',life:1.2});setTimeout(()=>{if(state==='play')this.spawn()},420)}
  },
  ultimateBreak(count){
    ...
    shake=25;flash=.8;zoomKick=.1;freeze=.15;audio.break(2.5);this.pickWeak();
    if(!this.floors.length){this.buildingId++;stage++;setTimeout(()=>{if(state==='play')this.spawn()},480)}
  },
  draw(c){
...
function resetGame(){
  ...
  particles.length=0;popups.length=0;waves.length=0;hazards.length=0;player.reset();building.reset();state='play';flash=.18;shake=4;
}
...
function update(dt){
  ...
  if(state==='menu'){
    ...
    tutorial=Math.max(0,tutorial-dt);player.perfectWindow=Math.max(0,player.perfectWindow-dt);player.update(dt);building.update(dt);
  ...
  } else if(state==='ultimate'){player.update(dt*.35);updateUltimate(dt)}
...
function drawUI(c){
  ...
  text('GUARD',44,H-50,17,'#9da6b5','left',false);c.fillStyle='#222936';c.fillRect(112,H-58,180,16);c.fillStyle=player.guardBroken>0?'#ef554d':'#5ed7ed';c.fillRect(112,H-58,180*(player.guardMeter/100),16);
  // score
  ...
  text(`BUILDING ${building.buildingId}  •  STAGE ${stage}`,W-28,30,19,'#d5d8df','right');
  if(tutorial>0&&state==='play'){const a=clamp(tutorial>1?1:tutorial,0,1);c.save();c.globalAlpha=a;c.fillStyle='rgba(0,0,0,.6)';c.beginPath();c.roundRect(W/2-330,112,660,72,18);c.fill();text('← → MOVE    ↑ JUMP    ↓ GUARD    Z ATTACK    X SPECIAL',W/2,142,24,'#ffffff');text('JUMP + Z  →  HIT THE GLOWING WEAK POINT!',W/2,174,18,'#ffd95a');c.restore()}
...
function drawHowTo(){
  ...
  const rows=[['← / →','MOVE','Position yourself under the weak point'],['↑','JUMP','Jump high enough to reach the building'],['↓','GUARD','Block falling debris / time it perfectly'],['Z','ATTACK','Best used in the air after jumping'],['X','SPECIAL','Use when the gauge reaches 100%']];
